...
export default async function AccountPage() {
  ...
  return (
    <div className="relative flex-1 overflow-hidden">
      <RotatingBackground images={[tonno, cappuccino, sfilettare]} />
      <div className="absolute inset-0 bg-gradient-to-b from-background/90 via-background/85 to-background" />

      <div className="relative z-10 mx-auto flex w-full max-w-2xl flex-col gap-8 px-6 py-16">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Ciao {customer.nome}</h1>
          <p className="mt-1 text-sm text-foreground/70">{customer.email}</p>
        </div>

        <div className="rounded-lg border border-foreground/20 p-5">
          <p className="text-sm font-medium text-foreground/70">Il tuo sconto attivo</p>
        ...
        <div className="rounded-lg border border-foreground/20 p-5">
          <p className="text-sm font-medium text-foreground/70">Invita un amico</p>
          <p className="mt-1 text-sm text-foreground/70">
            Se il tuo amico viene a cena, il tuo sconto sale al 15%.
          </p>
          <p className="mt-3 break-all rounded-md bg-foreground/5 px-3 py-2 font-mono text-sm text-foreground">
            {referralUrl}
          </p>
        ...
        <div>
          <p className="text-sm font-medium text-foreground/70">Le tue prenotazioni</p>
          <div className="mt-3 flex flex-col gap-2">
            {reservations && reservations.length > 0 ? (
              reservations.map((r) => (
                <div
                  key={r.id}
                  className="flex items-center justify-between rounded-md border border-foreground/20 bg-background/60 px-4 py-3 text-sm"
                >
                  <span>
                    {new Date(r.data).toLocaleDateString("it-IT")} · {r.orario} ·{" "}
                    {r.numero_persone} persone
                  </span>
              ...
              <p className="text-sm text-foreground/50">Nessuna prenotazione ancora.</p>
            )}
          </div>
        </div>
      </div>
